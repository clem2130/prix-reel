...
async function loadProviderComparison(city, offerType, speed, currentProvider, userPrice) {
  const comparisonBox = document.getElementById("provider-comparison");
  const comparisonList = document.getElementById("provider-comparison-list");
  const subtitle = document.getElementById("provider-comparison-subtitle");

  if (!comparisonBox || !comparisonList || !subtitle) return;
  ...
  try {
    let url =
      `${SUPABASE_URL}/rest/v1/internet_prices?select=Provider,Monthly_price` +
      `&City=eq.${encodeURIComponent(city)}` +
      `&Offer_type=eq.${encodeURIComponent(offerType)}` +
    ...
    const providers = Object.keys(grouped)
      .map(provider => {
        const average = grouped[provider].total / grouped[provider].count;
        const saving = Number(userPrice) - average;

        return {
          provider,
          average,
          count: grouped[provider].count,
          saving
        };
      })
    ...
    subtitle.textContent =
      "Prix moyens constatés pour des abonnements similaires dans votre ville.";
    ...
    providers.forEach(item => {
      ...
      card.innerHTML = `
        <div>
          ...
          <span>${item.count} prix observé${item.count > 1 ? "s" : ""}</span>
        </div>

        <div class="provider-price">
          <strong>${item.average.toFixed(0)} €</strong>
          <small>
            ${
              item.saving > 0
                ? `Économie possible : ${item.saving.toFixed(0)} €/mois`
                : `Prix similaire ou plus élevé`
            }
          </small>
        </div>
      `;
    ...
    console.error("Erreur comparaison fournisseurs :", error);
    comparisonBox.classList.add("hidden");
  }
}
